...
int main() {
    ...
    SoapySDRDevice_setGain(sdr, SOAPY_SDR_RX, channels[0], 35.0); // Чувствительность приемника
    ...
    // int16_t tx_buff[2*tx_mtu]; // Оригинал
    int16_t rx_buffer[2*rx_mtu];
    size_t count = 0;
    int16_t *tx_buff = read_pcm("Smesh.pcm", &count); 
    ...
    size_t iteration_count = count/1920;
    ...
    for (size_t buffers_read = 0; buffers_read < iteration_count; buffers_read++) {
        ...
        void *tx_buffs[] = {tx_buff + 1920 * buffers_read * 2};
        printf("\n");
        printf("%d\n", 1920 * buffers_read * 2);
         for(size_t i = 0; i < 2; i++) {
            tx_buff[0 + i] = 0xffff;
...
/*cmake_minimum_required(VERSION 3.16)   Оригинальный симэйк
project(PlutoSDR C)
